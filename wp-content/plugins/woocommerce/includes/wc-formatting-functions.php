...
function wc_price( $price, $args = array() ) {
	...
	$return          = '<span class="woocommerce-Price-amount amount test123"><bdi>' . $formatted_price . '</bdi></span>';
...
function wc_format_sale_price( $regular_price, $sale_price ) {
	$price = '<del>' . ( is_numeric( $regular_price ) ? wc_price( $regular_price ) : $regular_price ) . '</del> <ins>' . ( is_numeric( $sale_price ) ? wc_price( $sale_price ) : $sale_price ) . '</ins>'.'<span class="show-price-percentage">'. abs(number_format(($sale_price / $regular_price)*100 - 100 ,0)) .'%</span>';
