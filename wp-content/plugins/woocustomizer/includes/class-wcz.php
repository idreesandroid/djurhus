    ...
    public function wcz_pricechange_notice()
    {
        ...
        if ( current_user_can( 'manage_options' ) && !get_user_meta( $wcz_user_id, 'wcz_pricechange_notice_dismiss' ) && 'admin.php?page=wcz_settings&' == $wczpage ) {
            ...
            <div class="notice notice-info wcz-admin-notice wcz-price-notice">
                <h4>
                    <?php 
            esc_html_e( 'A heads up on StoreCustomizer Pro price change!', 'woocustomizer' );
            ?>
                </h4>
                <p>
                    <?php 
            /* translators: 1: 'WooCommerce Custom Product Badges feature', 2: 'purchase StoreCustomizer Pro now'. */
            printf( esc_html__( 'With the new upcoming features, StoreCustomizer Pro\'s price will increase slightly when the %1$s gets added... if you want to pay only $29... then %2$s. Oh... and use the coupon code "LASTCHANCE" and we\'ll give you a further 20&#x25; OFF !', 'woocustomizer' ), wp_kses( '<a href="' . admin_url( 'admin.php?page=wcz_settings&tab=wcz_product_badges' ) . '" class="wcz-admin-notice-a">' . __( 'WooCommerce Custom Product Badges feature', 'woocustomizer' ) . '</a>', array(
                'a' => array(
                'href'  => array(),
                'class' => array(),
            ),
            ) ), wp_kses( '<a href="' . admin_url( 'admin.php?billing_cycle=annual&page=wcz_settings-pricing' ) . '" class="wcz-admin-notice-a">' . __( 'purchase StoreCustomizer Pro now', 'woocustomizer' ) . '</a>', array(
                'a' => array(
                'href'  => array(),
                'class' => array(),
            ),
            ...
            ?></a> &nbsp; | &nbsp; <a href="<?php 
            echo  esc_url( admin_url( $wczpage . 'wcz_pricechange_notice_dismiss' ) ) ;
            ?>"><?php 
