...
function classiera_breadcrumbs(){
  /* === OPTIONS === */	
	...
	$text['blog']      = esc_html__('Blog','classiera');// text for the blog page
			...
			if(!empty($thisCat)){
				if(isset($thisCat->parent)){
					if ($thisCat->parent != 0) {
						...
						$cats  = get_term_parents_list($termID, $taxonomy, $taxargs);
						$cats = str_replace('<a', $linkBefore . '<a' . $linkAttr, $cats);
						$cats = str_replace('</a>', '</a>' . $linkAfter, $cats);
						echo wp_kses_post($before).$taxnomyCatLink.$after.$cats;						
					}else{
						
						$displayLink = 	$before.$taxnomyCatLink.$after;
						echo wp_kses_post($displayLink) . $before . $thisCat->name . $after;
					}
				}				
			}
