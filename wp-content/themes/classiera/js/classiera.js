...
jQuery(document).ready(function(jQuery){
    ...
    //active for grid list
    jQuery('.filter_section .view-as li').on('click', function(){
        if(jQuery(this).hasClass("active")!== true)
        {
            jQuery('.view-head .view-as li').removeClass("active");
            jQuery(this) .addClass("active");
    ...
    //list and grid view
    jQuery('.filter_section .view-as a').on('click', function(event){
        event.preventDefault();
        
        //add remove active class
        if(jQuery(this).hasClass("active")!== true)
        {
            jQuery('.filter_section .view-as a').removeClass("active");
            jQuery(this) .addClass("active");
        }
        ...
        var st = aclass.split(' ');
        var firstClass = st[0];        
        var selector = jQuery(this).closest('.filter_section').next().find('div.item');
        var classStr = jQuery(selector).attr('class'),
            lastClass = classStr.substr( classStr.lastIndexOf(' ') + 1);
        jQuery(selector)
        // Remove last class
...
jQuery(document).ready(function($){
    ...
    function filter_save(){
        ...
        var selectedValue = $(this). children("option:selected"). val();
    		if(selectedValue == 'lowered'){
    			$('#lowered_price'+id).show();
    			$('#all_ads').hide();
    			$('#cheapest_ads'+id).hide();
    			$('#all_ads'+id).hide();
    			$('#oldest_ads'+id).hide();
    			$('#expensive_ads'+id).hide();
    		}else if(selectedValue == 'oldest'){
    			$('#oldest_ads'+id).show();
    			$('#all_ads'+id).hide();
    			$('#lowered_price'+id).hide();
    			$('#cheapest_ads'+id).hide();
    			$('#expensive_ads'+id).hide();
    			...
    			$('#all_ads'+id).hide();
    			$('#oldest_ads'+id).hide();
    			$('#expensive_ads'+id).show();
    		}else if(selectedValue == 'cheapest'){
    			$('#lowered_price'+id).hide();
    			$('#all_ads'+id).hide();
    			$('#oldest_ads'+id).hide();
    			$('#expensive_ads'+id).hide();
    			$('#cheapest_ads'+id).show();
    		}else{
    			$('#lowered_price'+id).hide();
    			$('#all_ads'+id).show();
    			$('#oldest_ads'+id).hide();
	...
	jQuery(".filter_save").change(filter_save); 
});
...
jQuery(document).ready(function($){
    function classiera_remove_save_search(){
     ...
     if(search_id != ''){
			var data = {
				'action': 'classiera_remove_save_search',
				'search_id': search_id,
			};
			jQuery.post(ajaxurl, data, function(response){
				if(response){
    ...
    jQuery(".remove_save_search").click(classiera_remove_save_search);
});